...
The thermocouple to digital converter I use is a MAX6678. This is mostly because of the temp range, since it measures 0&deg;C to 1024&deg;C. I figured in a cooking situation, I wouldn't need temps lower than 0 and aluminum melts at 659&deg;C, so I could use it in a searing situation if I wanted. The MAX6678 use an SPI interface, which the Pico can talk to by specifying the pin assignments and using the MAX6675 library.
...
### Case
I saw these cheap clear hard cases on Ali Express and thought that it would make a great enclosure of the project. Since it was waterproof (at least according to the seller), I figured it would be fun to maintain that the best I could. To do this, I used some gaskets cable glands for the probes as well as a switch which had a rubber cover over it. Besides that, I just put some dielectric grease on the gasket of the case itself. The probes are also themselves waterproof.
...
The other code file, collect_and_graph.py, is a program which will ping the router IP (the Pico @ 192.168.1.1) and return the information like an API. This then gets written into a stored file for the duration of the cook. After the standing 'while' loop is terminated, the program creates a graph of the cook, with two lines that track the meat temp and the pit temp. I also added a patch between 225&deg;F and 275&deg;F which is a pretty standard window for smoking meat. 

I usually just run this file from a terminal window so I don't have to run a separate IDE. 

The resulting graph is also saved as a local image file. 

One parameter to pay particular attention to. The Pico updates its on-board variables every 2 seconds. Over a 12 hour cook, that's 21,600 data points, far more than is really useful. I typically sample every 20 seconds, or 3 times a minute instead. This is still plenty of data at a very useful resolution. However, due to the high update rate of the Pico, if you want to look at the numbers on the screen or refresh the API, the readings are more or less instant. 

---------------------------------------
## 3. Price Breakdown
I purchased almost everything from Ali Express, and here is an approximate price break down:
...
Additionally, there are a number of thing you can do to cut costs as well. There are a number microcontrollers which could be substituted, such as an ESP32, another RP2040 board, or an Arduino with something like an ESP8266. If you wanted a waterproof case but didn't want to get a fancy outdoor hard case, a watertight kitchen container was an option I considered. 
...
### C. Solar
One option I entertained is that since the power draw of the Pico, even with running a wireless AP, is so low, to charge up the battery with a solar panel during the day. It would also avoid having to open the box in order to charge it, which could be a cool feature. 

### D. Comparisons with Micropython
One of the reasons I wanted to do this project in C++ was becuase of the supposed performance advantages of the language compared to Micropython. However, this makes me curious if there's a concrete difference for this project if it were written in Micropython, since I'm not doing anything that's especially computation or power hungry. 
